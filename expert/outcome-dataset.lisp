...
(defun %outcome-dataset-example-json (database projection)
...
immutable products of the rule/expert versions recorded on each assertion."
  (multiple-value-bind (outcome limit scope rule-version include-superseded)
    ...
    (let* ((database (expert-host-database host))
      ...
      ;; Callers can choose a narrower declared filter in a later query version.
      (when (> (length candidates) +max-outcome-dataset-candidates+)
        (error "outcome_dataset_candidate_limit_exceeded: ~A"
               outcome))
      (let* ((filtered
               (remove-if-not
                (lambda (projection)
                  (%outcome-dataset-candidate-p
                   database projection scope rule-version include-superseded))
                candidates))
             (ordered
               (stable-sort
                (copy-list filtered) #'string<
        ...
        (%json-object
         ...
         (cons "examples"
               (mapcar (lambda (projection)
                         (%outcome-dataset-example-json database projection))
                       bounded))
