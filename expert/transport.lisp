...
(defun %rewrite-request (request upstreams)
  (let* ((header-end (%header-end request))
    ...
    (multiple-value-bind (method target version)
      ...
      (multiple-value-bind (forward-target upstream-url)
...
(defun %serve-http-client (client config)
  (unwind-protect
       (let ((client-stream (%socket-stream client)))
         (unwind-protect
              (multiple-value-bind (forward-request host port)
                  (%rewrite-request (%read-http-request client-stream)
                                    (runtime-config-upstreams config))
                       ...
                       (let ((upstream-stream (%socket-stream upstream)))
                         (unwind-protect
                              (progn
                                ...
                                (%copy-stream-octets upstream-stream client-stream))
