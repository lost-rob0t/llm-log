...
class QuantDetectionTest(unittest.TestCase):
    def test_detection_is_disabled_by_default(self):
        ...
        self.assertEqual(detect_output_anomalies(b"{}", b"not utf8 \xff", config), [])

    def test_parallel_streamed_tool_calls_are_reconstructed_by_index(self):
        request = json.dumps({"tools": [
        ...
        anomalies = detect_output_anomalies(request, response, QuantDetectionConfig(enabled=True))
    ...
    def test_repeated_reasoning_block_is_detected(self):
        ...
        response = json.dumps({"choices": [{"message": {"reasoning": repeated}}]}).encode()
        anomalies = detect_output_anomalies(b"{}", response, QuantDetectionConfig(enabled=True))
...
class SafeAlertStreamTest(unittest.IsolatedAsyncioTestCase):
    async def test_unknown_quant_plus_anomaly_emits_metadata_only_alert(self):
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            recorder = RoutingRecorder(root, init={"quant_detection": {"enabled": True}})
            ...
            event = CaptureEvent.from_bytes(
                event_id="evt-alert", provider="openrouter", upstream="https://openrouter.ai",
                method="POST", path="/api/v1/chat/completions", query="",
                request_headers={"Authorization": "Bearer secret-token"},
                request_body=json.dumps({"model": "z-ai/glm-5.3", "messages": [{"role": "user", "content": secret_prompt}]}).encode(),
                response_status=200, response_headers={}, response_body=response,
                started_at="2026-09-07T03:00:00+00:00", completed_at="2026-09-07T03:00:01+00:00", latency_ms=1000,
            )
